        ...
        with tqdm(list_records, unit="folder") as listFolder:
            for idx, folderInfo in enumerate(listFolder):
                ...
                for frame_idx in range(NUM_IMG_PER_SEQ):
                    data_point.append(
                        cv2.imread(f"{self.root_dir}/{folderName}/{folderName}_frame_{frame_idx + 1}.jpg")
                    )
...
splitDataset(train_ratio=0.8)

train_dataset = Violence_Drone_Dataset(train=True)
test_dataset = Violence_Drone_Dataset(train=False)